...
def tuning(normalize: bool, gs_config: Path | str) -> None:
    """
    ...
    for block_index_combination in tqdm(all_blocks_combination_indices):
        ...
        try:
            topics, _ = extractor.train(docs, normalize=normalize, embeddings=embeddings, y=y)
        except:
            print("Fit failed with exception.")
            continue
        else:
            assert np.array_equal(embeddings_c, embeddings), "Embeddings changed!"

            # DBCV score
            # distortion_score = extractor._topic_model.hdbscan_model.score(embeddings)
